(function () {
  ...
  const STYLE_PRESETS = {
    ko: [
      ...
      { name: "2D 애니 스타일", desc: "참고 이미지 그대로·둥근 얼굴·큰 눈·파스텔 한복·밝은 조선 마을", preview: "assets/style-ko-reference-2d-anime.png", tail: "exactly match the selected reference look: warm cheerful Korean folktale 2D animation, cute rounded faces with large sparkling eyes, tiny nose and mouth, slim graceful figures, fine warm-brown outlines, bright pastel hanbok with delicate traditional floral patterns, light soft cel shading, airy watercolor-like sunny Joseon village backgrounds, cream peach mint sky-blue palette, wholesome storybook mood, clean hand-drawn animation frame, no dark dramatic shadows, no photorealism, no 3D" },
  ...
  function migrateOldKoreanStyle(p) {
    if (p?.lang === "ko" && (/clean contemporary Korean historical 2D cel-animation/i.test(p.style || "") || /^warm cheerful Korean folktale 2D animation/i.test(p.style || ""))) {
  ...
  // 한국어(KO_KR)로 조선 황태후 목소리 새로 디자인 → 미리듣기 → 저장(publish)
  let _designedVoiceId = "";
  async function designInworldVoice() {
    const out = $("#prodDesignVoiceResult");
    const key = ($("#prodInworldKey")?.value || inworldKey()).trim().replace(/^Basic\s+/i, "");
    if (!key) { if (out) out.textContent = "Inworld 키를 먼저 넣으세요."; return; }
    if (out) out.textContent = "🎨 한국어 황태후 목소리 만드는 중… (10~20초)";
    try {
      const res = await apiFetch("https://api.inworld.ai/voices/v1/voices:design", {
        method: "POST", headers: { "content-type": "application/json", "authorization": "Basic " + key },
        body: JSON.stringify({
          voiceDesignConfig: { numberOfSamples: 1 },
          langCode: "KO_KR",
          designPrompt: "A dignified elderly Korean empress dowager, calm and measured, warm low-pitched female voice, refined royal authority, graceful unhurried pacing, clear articulation, gentle folktale narration tone.",
          previewText: "안녕하세요. 옛날 옛적, 깊은 산골 마을에 지혜로운 며느리가 살고 있었습니다. 그 집에는 오래도록 숨겨진 비밀이 하나 있었지요."
        })
      });
      const j = await res.json().catch(() => ({}));
      if (!res.ok) { if (out) out.textContent = "실패: " + String(j.message || JSON.stringify(j)).slice(0, 140); return; }
      const pv = (j.previewVoices || [])[0];
      if (!pv || !pv.previewAudio) { if (out) out.textContent = "미리듣기 오디오가 없어요. 다시 시도하세요."; return; }
      _designedVoiceId = pv.voiceId || pv.preview_id || "";
      new Audio("data:audio/wav;base64," + pv.previewAudio).play();
      if (out) {
        out.innerHTML = "▶ 들어보세요! 한국어로 잘 들리면 → ";
        const save = el("button", "btn sm btn-primary", "✅ 이 목소리로 저장");
        save.onclick = () => publishDesignedVoice(key);
        out.appendChild(save);
        const again = el("button", "btn sm", "↻ 다시 만들기");
        again.onclick = designInworldVoice;
        out.appendChild(again);
      }
    } catch (e) { if (out) out.textContent = "오류: " + String(e.message).slice(0, 100); }
  }
  async function publishDesignedVoice(key) {
    const out = $("#prodDesignVoiceResult");
    if (!_designedVoiceId) { if (out) out.textContent = "먼저 목소리를 만드세요."; return; }
    if (out) out.textContent = "저장 중…";
    try {
      const res = await apiFetch("https://api.inworld.ai/voices/v1/voices/" + encodeURIComponent(_designedVoiceId) + ":publish", {
        method: "POST", headers: { "content-type": "application/json", "authorization": "Basic " + key },
        body: JSON.stringify({ voiceId: _designedVoiceId, displayName: "황태후(한국어)", description: "조선 황태후 · 중년 · 차분 · 한국어", tags: ["ko", "folktale"] })
      });
      const j = await res.json().catch(() => ({}));
      const vid = j.voice?.voiceId || _designedVoiceId;
      if (!res.ok && !j.voice) { if (out) out.textContent = "저장 실패: " + String(j.message || JSON.stringify(j)).slice(0, 140); return; }
      if ($("#prodInworldVoiceKo")) $("#prodInworldVoiceKo").value = vid;
      localStorage.setItem(LS.inworldVoiceKo, vid);
      if (out) out.innerHTML = "✅ 저장됨! <b>한국어 Voice ID</b>에 넣었어요. 맨 아래 <b>저장</b>을 누른 뒤 음성 생성하세요.";
    } catch (e) { if (out) out.textContent = "오류: " + String(e.message).slice(0, 100); }
  }
...
JSON만: {"video":"..."}`;
  ...
  function init() {
    ...
    if ($("#prodDesignVoice")) $("#prodDesignVoice").onclick = designInworldVoice;
